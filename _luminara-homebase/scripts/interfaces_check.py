...
def determine_status(block_height, latest_block, service_conf):
    if not service_conf or latest_block == 0:
        return "Down"
...
# Structure output with ordered fields
output_data = {
    "script_start_time": START_TIME,
    "script_end_time": "",
    "reference_latest_block_height": "0",
    "required_versions": {
...
# First pass: collect all block heights
all_block_heights = []
...
for network, sources in INTERFACES.items():
    ...
    for interface in interfaces:
        ...
        # Collect valid block heights
        for s in settings:
            if s.get("latest_block_height", "").isdigit():
                all_block_heights.append(int(s["latest_block_height"]))

# Calculate global maximum block height
global_max_block = max(all_block_heights) if all_block_heights else 0
output_data["reference_latest_block_height"] = str(global_max_block)

# Second pass: process interfaces with known global maximum
for network, sources in INTERFACES.items():
    interfaces_json = fetch_url(sources["interface"], timeout=5)
    if not interfaces_json:
        continue
    ...
    for interface in interfaces:
        ...
        # Create interface entry with ordered fields
        interface_entry = {
